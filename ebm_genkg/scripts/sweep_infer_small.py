...
OUTDIR=/home/code/3Ddetection/IS-Fusion/GenKG/ebm_genkg/tmp/sweep_ebm
mkdir -p "$OUTDIR"
...
def main():
    ...
    # EBM params（按你 InferConfig / ebm.py 里的字段名来对齐）
    ap.add_argument("--ebm_steps", type=int, default=30)
    ap.add_argument("--ebm_lr", type=float, default=0.05)
    ap.add_argument("--ebm_temp", type=float, default=1.0)
    ap.add_argument("--ebm_w_det", type=float, default=1.0)
    ap.add_argument("--ebm_w_attr", type=float, default=0.5)
    ap.add_argument("--ebm_w_repulse", type=float, default=0.2)
    ap.add_argument("--ebm_repulse_thr", type=float, default=1.5)
    ...
    for keep_thr in keep_thrs:
        for nms_thr in nms_thrs:
            for topk in topks:
                ...
                # EBM
                ebm_steps=int(args.ebm_steps),
                ebm_lr=float(args.ebm_lr),
                ebm_temp=float(args.ebm_temp),
                ebm_w_det=float(args.ebm_w_det),
                ebm_w_attr=float(args.ebm_w_attr),
                ebm_w_repulse=float(args.ebm_w_repulse),
                ebm_repulse_thr=float(args.ebm_repulse_thr),
                )
                ...
                row = {
                    ...
                    "two_stage": int(bool(args.two_stage)),
                    "seed_keep_thr": float(args.seed_keep_thr),
                    "fill_keep_thr": float(args.fill_keep_thr),
                    "min_dt_support": int(args.min_dt_support),
                    "dt_cell_size": float(args.dt_cell_size),
