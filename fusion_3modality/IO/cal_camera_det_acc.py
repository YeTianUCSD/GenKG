...
'''
python /home/code/3Ddetection/IS-Fusion/GenKG/fusion_3modality/IO/cal_camera_det_acc.py \
  --camera /home/code/3Ddetection/IS-Fusion/GenKG/fusion_3modality/IO/output/camera__score0p005.json \
...
# -------------------------- Matching utilities (copied from your cal_det_acc.py) ------------------------

def to_int_list(xs: List[Any]) -> List[int]:
    # Convert labels to ints; strings will be hashed
    out = []
    for v in xs or []:
        try:
            out.append(int(v))
...
def extract_centers(boxes: List[List[float]], use_3d: bool) -> List[Tuple[float, float, float]]:
    # Extract (x,y[,z]) centers; if use_3d=False, z is ignored
    centers = []
    for b in boxes or []:
...
# --------------------------- Metric helpers (copied from your cal_det_acc.py) ---------------------------
...
# --------------------------- Sample traversal (copied from your cal_det_acc.py) -------------------------
...
# ------------------------- Build flat samples from camera + GT -----------------------------

def build_flat_samples_camera_with_gt(
    camera_root: Any,
    ...
    det_frame:
      - "global": det.translation is in global, convert to lidar using GT poses (recommended)
      - "lidar":  det.translation already in lidar, no conversion
      - "ego":    det.translation in ego, convert to lidar using lidar2ego only
    """
    if not isinstance(camera_root, list):
        raise TypeError(f"Expected camera JSON top-level to be a list, but got: {type(camera_root)}")
...
# ------------------------------ Main eval (same output format) -----------------------------

def evaluate_dataset(root_obj: Any, thresholds: List[float], use_3d_center: bool, ignore_classes: set):
...
def parse_ignore_classes(s: Optional[str]) -> set:
    # Parse ignore classes like "-1,255"
    if not s:
        return set()
    out = set()
    for tok in s.split(","):
...
def main():
    ap = argparse.ArgumentParser(description="Evaluate camera detections by aligning GT via sample_token and fixing coordinate frame.")
    ap.add_argument("--camera", required=True, help="Path to camera detections JSON/JSONL (optionally .gz).")
    ap.add_argument("--camera-format", choices=["auto", "json", "jsonl"], default="auto", help="Force camera input format.")
    ...
    ap.add_argument("--score-thr", type=float, default=None, help="Optional additional score threshold applied to camera detections.")
    ...
    ap.add_argument("--det-frame", choices=["global", "lidar", "ego"], default="global",
                    help="Coordinate frame of camera detection translation. Default: global (will convert to lidar using GT poses).")
    ap.add_argument("--ignore-classes", default=None, help="Comma-separated class IDs to ignore (e.g., -1).")
    ...
    cam_fmt = args.camera_format if args.camera_format != "auto" else detect_format_from_content(args.camera)
    camera_root = load_root(args.camera, cam_fmt)
    ...
    ignore_classes = parse_ignore_classes(args.ignore_classes)

    flat_samples, align_stats = build_flat_samples_camera_with_gt(
    ...
    print("-" * 80)
    print(f"Camera unique sample_tokens: {align_stats['det_unique_tokens']}")
    ...
    print(f"Missing in camera:           {align_stats['missing_in_det']}")
    print(f"Camera detections raw/kept:  {align_stats['total_kept_det']} / {align_stats['total_raw_det']}"
    ...
    if ignore_classes:
        print(f"Ignored classes:             {sorted(ignore_classes)}")
    print("=" * 80)

    stats, classes_seen, n_samples, n_det, n_gt = evaluate_dataset(
        flat_samples, thresholds, args.use_3d_center, ignore_classes
    )
