...
"""
时序版“检测天花板召回率”统计脚本（带 max_per_frame 截断）：
...
新增：
- --max_per_frame：每帧最多保留多少个 det（按 scores_3d 从大到小截断），
  用来模拟训练脚本里的 max_per_frame。
  例如 --max_per_frame 150 表示“每帧只用前 150 个得分最高的框”；
  0 或 None 表示不截断，使用所有 det。

用法示例：

python -u /home/code/3Ddetection/IS-Fusion/GenKG/code/cal_dataset/check_detection_maxV3.py \
