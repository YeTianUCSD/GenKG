...
python /home/code/3Ddetection/IS-Fusion/GenKG/code/convert_isfusionres_gt_attribution2json_train.py \
  --infos   /home/code/3Ddetection/IS-Fusion/GenKG/data/nusence_train/nuscenes_infos_train_sorted.pkl \
  --results /home/code/3Ddetection/IS-Fusion/GenKG/data/nusence_train/nuscenes_train_results.pkl \
  --dataroot /home/dataset/nuscene \
  --version v1.0-trainval \
...
# --------------------- 兼容 numpy._core 的 Unpickler --------------------- #
class NumpyCoreFixUnpickler(pickle.Unpickler):
    """
    兼容由 numpy>=2.0 生成的 pickle:
    那些 pickle 里把内部模块记成 numpy._core.xx，
    在 numpy 1.x 环境下没有这个模块，需要映射回 numpy.core.xx。
    """
    def find_class(self, module, name):
        # 关键：把 numpy._core.xxx 映射到 numpy.core.xxx
...
def load_pickle_compat(path: str):
    """
    使用 NumpyCoreFixUnpickler 来加载 pickle，
    解决 numpy 2.x/1.x 之间的 pickle 兼容问题。
    """
    with open(path, "rb") as f:
        return NumpyCoreFixUnpickler(f).load()
# ------------------------------------------------------------------------ #
...
def main():
    ...
    # ---------- 使用兼容 loader 读取 infos/result ---------- #
    infos_obj = load_pickle_compat(args.infos)
    ...
    results = load_pickle_compat(args.results)
