...
python -u infer_det_transformer_use_model.py \
  ...
  --out_dir /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/infer_single \
  --frame_radius 2 \
  --match_thr 1 \
  --max_per_frame 200 \
  --keep_thr 0.5

示例用法（搜索多组 keep_thr & max_per_frame）：

python -u /home/code/3Ddetection/IS-Fusion/GenKG/code/model/infer_det_transformer_use_modelV2.py \
  --ckpt /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/search_res/best_overall.pt \
  --input_json /home/code/3Ddetection/IS-Fusion/GenKG/data/nusence_val/sorted_by_scene_ISFUSIONandGTattr_val.json \
  --out_dir /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/infer_res \
  --frame_radius 2 \
  --match_thr 1
  --keep_thr_list 0.1, 0.3, 0.5,0.7, 0.9 \
  --max_per_frame_list 200,150,100,50


LOGFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/infer_res_V2/infer_search.log
PIDFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/infer_res_V2/infer_search.pid

setsid nohup python -u /home/code/3Ddetection/IS-Fusion/GenKG/code/model/infer_det_transformer_use_modelV2.py \
  --ckpt /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/search_res/best_overall.pt \
  --input_json /home/code/3Ddetection/IS-Fusion/GenKG/data/nusence_val/sorted_by_scene_ISFUSIONandGTattr_val.json \
  --out_dir /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/infer_res_V2 \
  --frame_radius 2 \
  --match_thr 1 \
  --keep_thr_list 0.7,0.5,0.3, \
  --max_per_frame_list 50,30,10 \
  > "$LOGFILE" 2>&1 & printf '%s\n' "$!" > "$PIDFILE"


删除：
pkill -TERM -f infer_det_transformer_use_modelV2.py

# 1) 杀掉搜索脚本本身
kill "$(cat /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/infer_res/infer_search.pid)"
ps aux | grep infer_det_transformer_use_modelV2.py | grep -v grep
"""
...
@torch.no_grad()
def evaluate_prf(model, loader, device, keep_thr: float):
    """
    使用 Dataset 中预生成的 center_keep_tgt（和训练脚本同逻辑），
    在给定 keep_thr 下计算 P/R/F1。
    """
    model.eval()
    ...
    for batch in loader:
        ...
        tot["tp"] += float(((pred_keep == 1) & (tgt_keep == 1)).sum().item())
        tot["fp"] += float(((pred_keep == 1) & (tgt_keep == 0)).sum().item())
        tot["fn"] += float(((pred_keep == 0) & (tgt_keep == 1)).sum().item())
...
def main():
    ...
    ap.add_argument("--match_thr", type=float, default=2.0,
                    help="用于生成 GT keep 标签的匹配阈值，应与训练时一致")
    ...
    print(f"[INFO] keep_thr_list      = {keep_thr_list}")
    print(f"[INFO] max_per_frame_list = {max_per_frame_list}")
    ...
    # 2) Load model
    print("[2/4] Loading model from ckpt...")
    ...
    print("[3/4] Searching over (max_per_frame, keep_thr) ...")
    ...
    for mpf in max_per_frame_list:
        print(f"\n[INFO] Building dataset for max_per_frame={mpf} ...")
        ...
        for kth in keep_thr_list:
            ...
            metrics = evaluate_prf(model, loader, device, keep_thr=kth)
