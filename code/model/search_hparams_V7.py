...
"""
网格搜索超参组合：
  (frame_radius, match_thr, weight_decay, max_tokens, assoc_thr)

每组参数：
  - 启动 train_det_transformer_V7.py
  - 训练若干个 epoch
...
同时：
  - 每组参数的训练输出都会「实时」打印到终端，
    并写到该组合自己的 log 文件里：
      BASE_OUT_DIR/r{R}_mt{MT}_wd{WD}_tk{TK}_at{AT}/train_r{R}_mt{MT}_wd{WD}_tk{TK}_at{AT}.log
  - 全局维护一个 best F1：
...
用法示例：
    ...
    python /home/code/3Ddetection/IS-Fusion/GenKG/code/model/search_hparams_V7.py

长期运行：

    cd /home/code/3Ddetection/IS-Fusion/GenKG/code/model

    PIDFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_v7.pid
    LOGFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_v7.log
    setsid nohup python /home/code/3Ddetection/IS-Fusion/GenKG/code/model/search_hparams_V7.py \
      >> "$LOGFILE" 2>&1 & printf '%s\n' "$!" > "$PIDFILE"

停止：

    pkill -TERM -f search_hparams_V7.py
    # 或者：
    kill "$(cat /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_v7.pid)"
...
TRAIN_SCRIPT = "/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer_V7.py"
...
# 所有搜索结果统一放在这个目录下面
BASE_OUT_DIR = "/home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV7/search_res_V1"
...
# ======== 超参搜索空间 ========
# 你可以随时改这些 list 的取值；长度为 1 时，相当于这个维度不做搜索，只是显式固定

# 时间窗口半径
FRAME_RADIUS_LIST = [9,7,5]

# det 与 gt 的匹配距离阈值（训练时构造 label 用）
MATCH_THR_LIST    = [2, 3, 5]

# AdamW weight_decay
WEIGHT_DECAY_LIST = [1e-3]

# 新增：窗口融合后，送进 Transformer 的 proposal 上限（全局 top-M）
MAX_TOKENS_LIST   = [200]   # 先固定一个值；需要的话可以比如 [200, 300]

# 新增：窗口内 det 时序关联的 xy 阈值（聚合 tracklet 用）
ASSOC_THR_LIST    = [2.0]   # 先固定一个值；需要的话可以例如 [1.5, 2.0, 3.0]
...
def run_one_trial(frame_radius: int,
                  ...
                  assoc_thr: float):
    """
    跑一组 (frame_radius, match_thr, weight_decay, max_tokens, assoc_thr) 组合。
    返回：
      {
        "best_f1": ...,
        "best_epoch": ...,
    ...
    """
    # 浮点转 tag：把 '.' 换成 'p'，方便拼目录名
    mt_tag = f"{match_thr}".replace(".", "p")
    wd_tag = f"{weight_decay}".replace(".", "p")
    at_tag = f"{assoc_thr}".replace(".", "p")
    tk_tag = f"{max_tokens}"

    out_dir = os.path.join(
        BASE_OUT_DIR,
        f"r{frame_radius}_mt{mt_tag}_wd{wd_tag}_tk{tk_tag}_at{at_tag}"
    )
    os.makedirs(out_dir, exist_ok=True)

    # log 文件名里带上参数，方便区分
    log_filename = f"train_r{frame_radius}_mt{mt_tag}_wd{wd_tag}_tk{tk_tag}_at{at_tag}.log"
    ...
    cmd = [
        ...
        # 新增：显式传 max_tokens / assoc_thr，而不是依赖训练脚本默认值
        "--max_tokens",    str(max_tokens),
        "--assoc_thr",     str(assoc_thr),
    ]

    print(f"\n=== Run: frame_radius={frame_radius}, match_thr={match_thr}, "
          f"weight_decay={weight_decay}, max_tokens={max_tokens}, assoc_thr={assoc_thr} ===")
...
def main():
    ...
    for frame_radius, match_thr, weight_decay, max_tokens, assoc_thr in product(
        FRAME_RADIUS_LIST,
        MATCH_THR_LIST,
        WEIGHT_DECAY_LIST,
        MAX_TOKENS_LIST,
        ...
        if best_epoch >= 0 and best_f1 > global_best_f1:
            ...
            print(
                f"New global best F1: {best_f1:.4f} "
                f"(radius={frame_radius}, match_thr={match_thr}, "
                f"weight_decay={weight_decay}, max_tokens={max_tokens}, "
                f"assoc_thr={assoc_thr}, epoch={best_epoch})"
            )
    ...
    for i, r in enumerate(all_results[:5]):
        print(
            ...
            f"max_tokens={r['max_tokens']} assoc_thr={r['assoc_thr']} | "
            f"log={r['log_path']}"
        )
    ...
    if global_best_info is not None:
        ...
        print(
            ...
            f"assoc_thr={global_best_info['assoc_thr']}"
        )
