...
同时：
  ...
  - 全局维护一个 best F1：
    ...
    python /home/code/3Ddetection/IS-Fusion/GenKG/code/model/search_hparams_V4.py
...
setsid nohup python /home/code/3Ddetection/IS-Fusion/GenKG/code/model/search_hparams_V4.py \
  >> "$LOGFILE" 2>&1 & printf '%s\n' "$!" > "$PIDFILE"


删除：
pkill -TERM -f search_hparams_V4.py

# 1) 杀掉搜索脚本本身
kill "$(cat /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_v4.pid)"
ps aux | grep train_det_transformer_V4.py | grep -v grep
...
TRAIN_SCRIPT = "/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer_V4.py"
...
# 所有搜索结果统一放在这个目录下面
BASE_OUT_DIR = "/home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV4/search_res"

# 使用多少卡（跟你平时 torchrun 一致）
NPROC  = 2
EPOCHS = 10      # 你截图里是 10，可以按需改
