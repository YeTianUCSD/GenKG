...
torchrun --nproc_per_node=2 /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer_V3.py \
  ...
  --out_dir    /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV3/ \
...
多卡「永久执行」版本
PIDFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_V3.pid
LOGFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_V3.log

setsid nohup torchrun --nproc_per_node=2 /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer_V3.py \
  --train_json /home/code/3Ddetection/IS-Fusion/GenKG/code/model/dataset/sorted_by_scene_ISFUSIONres_GT_attributions_train.json \
  --val_json   /home/code/3Ddetection/IS-Fusion/GenKG/code/model/dataset/sorted_by_scene_ISFUSIONandGTattr_val.json \
  --out_dir    /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV3/ \
  --epochs 100000000 --match_thr 2.0 --keep_thr 0.5 --ignore_classes -1 \
  --max_per_frame 200 \
  --log_interval 20 >> "$LOGFILE" 2>&1 & printf '%s\n' "$!" > "$PIDFILE"


单卡：
python /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer_V3.py \
  --train_json /home/code/3Ddetection/IS-Fusion/GenKG/code/model/dataset/sorted_by_scene_ISFUSIONres_GT_attributions_train.json \
  --val_json   /home/code/3Ddetection/IS-Fusion/GenKG/code/model/dataset/sorted_by_scene_ISFUSIONandGTattr_val.json \
  --out_dir    /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV3_1gpu \
  --epochs 8 --match_thr 2.0 --keep_thr 0.5 --ignore_classes -1 \
  --max_per_frame 200 \
  --log_interval 50
...
def main():
    ...
    train_dataset_for_sampler = ClipDataset(
        train_scenes, frame_radius=2, max_per_frame=args.max_per_frame,
        match_thr=args.match_thr, ignore_classes=ignore
    )
    val_dataset_for_sampler = ClipDataset(
        val_scenes, frame_radius=2, max_per_frame=args.max_per_frame,
        match_thr=args.match_thr, ignore_classes=ignore
    )
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if new_best:
            if is_main_process():
                print(f"[E{epoch}] New best, running inference on val to generate pred_flat.json & pred_full.json ...")

            # 让 val sampler 有一个固定 epoch（仅在 ddp 时影响顺序）
            if ddp and isinstance(val_loader.sampler, DistributedSampler):
                val_loader.sampler.set_epoch(1000 + epoch)

            rank_id = dist.get_rank() if ddp else 0
            shard_flat = os.path.join(args.out_dir, f"pred_flat.rank{rank_id}.json")
            shard_repl = os.path.join(args.out_dir, f"repl.rank{rank_id}.json")

            predict_shard(
                model.module if isinstance(model, DDP) else model,
                val_loader, device, args.keep_thr, shard_flat, shard_repl
            )
            barrier()

            if is_main_process():
                pred_path_flat = os.path.join(args.out_dir, "pred_flat.json")
                pred_path_full = os.path.join(args.out_dir, "pred_full.json")

                # 合并 flat
                flat_all = []
                for r in range(world_size):
                    path = os.path.join(args.out_dir, f"pred_flat.rank{r}.json")
                    if os.path.exists(path):
                        with open(path, "r") as f:
                            flat_all.extend(json.load(f))
                with open(pred_path_flat, "w") as f:
                    json.dump(flat_all, f, ensure_ascii=False, indent=2)
                print(f"[E{epoch}] [OK] Wrote flat val predictions to: {pred_path_flat}")

                # 合并 repl 并写结构保留版（基于 val_root）
                ...
                root_copy = copy.deepcopy(val_root)
                ...
                print(f"[E{epoch}] [OK] Wrote full-structure val predictions to: {pred_path_full}")

                # 可选：合并后清理分片
                if not args.keep_shards:
                    for r in range(world_size):
                        for name in (f"pred_flat.rank{r}.json", f"repl.rank{r}.json"):
