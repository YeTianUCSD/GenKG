...
"""
DDP version with mid-epoch loss logging:
...
# 多卡（例如 4 卡），每 20 step 打印一次 Loss
torchrun --nproc_per_node=2 /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer.py \
  --input_json /home/code/3Ddetection/IS-Fusion/GenKG/data/nusence_val/sorted_by_scene_ISFUSIONandGTattr.json \
  --out_dir    /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV2/ \
  --epochs 8 --match_thr 2.0 --keep_thr 0.5 --ignore_classes -1 \
  --max_per_frame 200 \
  --log_interval 20

PIDFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train.pid
LOGFILE=/home/code/3Ddetection/IS-Fusion/GenKG/code/model/train.log
setsid nohup torchrun --nproc_per_node=2 /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train_det_transformer.py \
  --input_json /home/code/3Ddetection/IS-Fusion/GenKG/data/nusence_val/sorted_by_scene_ISFUSIONandGTattr.json \
  --out_dir    /home/code/3Ddetection/IS-Fusion/GenKG/code/model/outV2/ \
...
kill -15 "$(cat /home/code/3Ddetection/IS-Fusion/GenKG/code/model/train.pid)"
pkill -TERM -f train_det_transformer.py

# 单卡
python /path/to/train_det_transformer_ddp_pad_log.py \
  --input_json .../sorted_by_scene_ISFUSIONandGTattr.json \
  --out_dir    .../out_1gpu \
  --epochs 8 --match_thr 2.0 --keep_thr 0.5 --ignore_classes -1 --log_interval 50
...
class ClipDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if keep_idx_c.size > 0 and keep_idx_g.size > 0:
            ...
            k_det_xyz = center_xyz[keep_idx_c]; k_gt_xyz  = gt_boxes[keep_idx_g, :3]
            k_det_lab = center_labels[keep_idx_c]; k_gt_lab  = gt_labels[keep_idx_g]
...
def main():
    ...
    if is_main_process():
        ...
        if not args.keep_shards:
            for r in range(world_size):
                for name in (f"pred_test.rank{r}.json", f"repl.rank{r}.json"):
